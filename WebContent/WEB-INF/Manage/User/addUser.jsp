	...
	<form action="" method="post" class="form form-horizontal" id="form-member-add" enctype="multipart/form-data">
		...
		<div class="row cl" id="etime">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text"  placeholder="请输入用户名(5-12位)" id="account" name="account" style="width:500px;">
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="password" class="input-text" placeholder="请输入密码(5-16位)" id="password" name="password" style="width:500px;">
			</div>
		</div>
		
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-3">
				<input class="btn btn-danger radius" type="button" value="&nbsp;&nbsp;确定添加&nbsp;&nbsp;" onclick="addUser()">
			</div>
		</div>
...
function addUser(){
	if($("#account").val()==""){layer.alert("请输入用户名！"); return;} 
	...
	$.ajax({
	  ...
	  success: function(data) {
		  if(data==1){
			  layer.confirm("添加帐号成功！",{btn:['返回帐号列表','继续添加']},function(index){
	       		 	layer_close();
           });
		  }else{
			  layer.alert("添加失败！");
